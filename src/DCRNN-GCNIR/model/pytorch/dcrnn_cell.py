...
class DCGRUCell(torch.nn.Module):
    def __init__(self, num_units, adj_mx, max_diffusion_step, num_nodes, nonlinearity='tanh',
        ...
        elif filter_type == "s_aorm":
            # supports.append(utils.calculate_s_arom(adj_mx).T, order = self._max_diffusion_step)
            supports.append(utils.calculate_s_arom(adj_mx).T)
        else:
    ...
    def forward(self, inputs, hx):
        ...
        # print(f'# Total diffusion time: {round(self.total_diffusion_time,3)} secs')
    ...
    def _gconv(self, inputs, state, output_size, bias_start=0.0):
        ...
        if self._max_diffusion_step == 0:
            ...
            diffusion_time = round(finish-start, 3)
            self.total_diffusion_time += diffusion_time
